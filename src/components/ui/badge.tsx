    ...
    <span
      className={cn(
        "inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium transition-colors",
        {
          ...
          "bg-blue-600/20 text-blue-400 border border-blue-500/30": variant === "primary",
...
function SkillBadge({ className, ...props }: HTMLAttributes<HTMLSpanElement>) {
  return (
    <span
      className={cn(
        "px-3 py-1 bg-slate-800 rounded-full text-sm text-slate-300",
        className
      )}
      {...props}
    />
...
function TagBadge({ className, ...props }: HTMLAttributes<HTMLSpanElement>) {
  return (
    <span
      className={cn(
        "px-2 py-0.5 bg-slate-700 rounded text-xs text-slate-300",
        className
      )}
      {...props}
    />
